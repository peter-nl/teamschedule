...
export class AddTeamDialogComponent implements OnInit {
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      this.notificationService.success(this.translate.instant('addTeam.messages.success'));
      this.panelRef.close(true);
    } catch (error: any) {
      console.error('Failed to add team:', error);
      this.notificationService.error(error.message || this.translate.instant('addTeam.messages.failed'));
    } finally {
      this.loading = false;
    }
  }
